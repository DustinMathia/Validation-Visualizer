...
def make_roc_curve(positive_data, negative_data, unknown_data):  # view confusion matrix chart @ https://en.wikipedia.org/wiki/Receiver_operating_characteristic
  ...
  population_data = sorted(positive_tuples + negative_tuples + unknown_tuples, key=lambda x: x[0])
...
def gen_roc_table(roc_data, threshold_value, norm_params):
    if not roc_data or not roc_data.get('population_data'):
        ...
        return [[None,None,None,None],
                [None,None,None,None],
                [None,None,None,None]], roc_table_header, 0
    ...
    tp_val = (roc_data['total_positive'] - fn_val) if roc_data['total_positive'] is not None else 0
    fp_val = (roc_data['total_negative'] - tn_val) if roc_data['total_negative'] is not None else 0
    up_val = (roc_data['total_unknown'] - un_val) if roc_data['total_unknown'] is not None else 0

    tpr_val = round(tp_val / roc_data['total_positive'], 2) if roc_data['total_positive'] > 0 else 0
    fpr_val = round(fp_val / roc_data['total_negative'], 2) if roc_data['total_negative'] > 0 else 0
    tnr_val = round(tn_val / roc_data['total_negative'], 2) if roc_data['total_negative'] > 0 else 0 # Specificity
    fnr_val = round(fn_val / roc_data['total_positive'], 2) if roc_data['total_positive'] > 0 else 0 # Miss Rate

    total_classified = (roc_data['total_positive'] or 0) + (roc_data['total_negative'] or 0)
    acc_val = round((tp_val + tn_val) / total_classified, 2) if total_classified > 0 else 0
